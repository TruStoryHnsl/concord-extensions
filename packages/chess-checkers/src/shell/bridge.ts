 ...
 *
 * Mirrors packages/card-suite/src/shell/bridge.ts. When the SDK is
...
/**
 * Dev fallback default. mode = shared_admin_input → party so the picker
 * shows both games + bot tier; seat = participant so the local player
 * gets controller surface (active pieces) instead of read-only display.
 */
