...
**Tech Stack:** CesiumJS, vanilla JS (IIFE-wrapped, implicit globals via `window.WV.*`), CSS custom properties + backdrop-filter, GeoJSON, Google Map Tiles API, FastAPI (concord proxy), token-bucket rate limiting, sessionStorage / localStorage. **All DOM construction uses createElement + textContent — never assign HTML strings to innerHTML for any data that could include user-influenced text.**

**Reference spec:** `docs/superpowers/specs/2026-04-30-worldview-photoreal-design.md`

**Source-of-truth path:** `concord-extensions/packages/worldview-map/`. The source layout is:
- `src/` — JS source (layers, ui, lib, models, config)
- `styles/main.css`
- `index.html`
- `manifest.json`
- `package.json`, `scripts/build.mjs`, `scripts/pack.mjs`

All file paths in this plan are relative to `packages/worldview-map/` unless otherwise noted.

**Build pipeline:** `dist/` is BUILD OUTPUT, regenerated by `node scripts/build.mjs` (alias `pnpm build`). The build copies `src/`, `styles/`, `index.html`, `manifest.json` into `dist/`, then rewrites every `/proxy/<provider>/` reference in JS files to `/api/ext-proxy/com.concord.worldview-map/<provider>/` (the runtime path concord exposes), and injects a config-loader bridge that populates `window.WV.config` from localStorage before any layer JS runs.

Practical implications:
- Edit files in `src/`, `styles/`, `index.html`, `manifest.json`. Never edit `dist/` — it gets overwritten on every build.
- Use `/proxy/<source>/...` paths in JS source (the build rewrites them at packaging).
- After every edit, run `node scripts/build.mjs` (or `pnpm build` if pnpm is available) before serving `dist/` for browser verification.
- Tokens live in `localStorage` keyed `wv.<KEY>` (e.g. `wv.GOOGLE_MAPS_KEY`), NOT in a checked-in `config.js`. The build's config-loader reads `localStorage.getItem("wv.<KEY>")` and assigns it to `window.WV.config.<KEY>`. To set a key in dev, run in DevTools console: `localStorage.setItem('wv.GOOGLE_MAPS_KEY', 'AIza...')` then reload.
- Verification serve command: from `packages/worldview-map/`, run `node scripts/build.mjs && python3 -m http.server -d dist 8088`.
...
```bash
ls ~/projects/worldview-wt/packages/worldview-map/src/main.js
ls ~/projects/worldview-wt/packages/worldview-map/src/layers/ | wc -l
```
...
### Task A1: Source registry — `src/lib/sources.js`

The single source of truth for every data source: URL, auth mode, refresh interval, daily cap, transport, proxy-required flag.

**Files:**
- Create: `packages/worldview-map/src/lib/sources.js`

- [ ] **Step 1: Create the directory**

```bash
mkdir -p ~/projects/worldview-wt/packages/worldview-map/src/lib
```
...
```bash
cd ~/projects/worldview-wt
git add packages/worldview-map/src/lib/sources.js packages/worldview-map/index.html
git commit -m "feat(worldview): add source registry"
```

---

### Task A2: Egress wrapper — `src/lib/wv-fetch.js`

**Files:**
- Create: `packages/worldview-map/src/lib/wv-fetch.js`
...
```bash
cd ~/projects/worldview-wt
git add packages/worldview-map/src/lib/wv-fetch.js packages/worldview-map/index.html
git commit -m "feat(worldview): add wv-fetch egress wrapper"
```

---

### Task A3: Photoreal install — `src/lib/photoreal.js`

**Files:**
- Create: `packages/worldview-map/src/lib/photoreal.js`
...
- [ ] **Step 4: Add key slot to `src/config.example.js`** for documentation:

Append to the existing `WV.config = {…}` block:
```javascript
  GOOGLE_MAPS_KEY: 'YOUR_GOOGLE_MAPS_KEY_HERE',  // restricted to: Map Tiles, Geocoding, Places, AQ, Pollen
```

- [ ] **Step 5: Verify build's config-loader bridges the key from localStorage**

The build (`scripts/build.mjs`) injects a config-loader before `main.js` that reads `localStorage.getItem('wv.<KEY>')` and assigns to `window.WV.config.<KEY>`. Confirm `GOOGLE_MAPS_KEY` is in the build's known-keys list. If not, add it:

```bash
grep -n 'GOOGLE_MAPS_KEY\|wv\\.' ~/projects/worldview-wt/packages/worldview-map/scripts/build.mjs
```

If the build doesn't bridge `GOOGLE_MAPS_KEY` automatically, edit `scripts/build.mjs` to include it in the config-loader generation list. Otherwise the key won't reach the runtime even after the user puts it in localStorage.

The user sets the key once via DevTools console after build:
```javascript
localStorage.setItem('wv.GOOGLE_MAPS_KEY', '<paste user key>')
```
...
```bash
cd ~/projects/worldview-wt
git add packages/worldview-map/src/lib/photoreal.js \
        packages/worldview-map/src/main.js \
        packages/worldview-map/src/config.example.js \
        packages/worldview-map/index.html \
...
### Task A4: Steel palette — `styles/main.css`

**Files:**
- Modify: `packages/worldview-map/styles/main.css:1-100`
...
```bash
cd ~/projects/worldview-wt
git add packages/worldview-map/styles/main.css \
...
### Task A5: Stream-health panel — `src/ui/health-panel.js`

DOM construction via `createElement` + `textContent` — no innerHTML.

**Files:**
- Create: `packages/worldview-map/src/ui/health-panel.js`
- Modify: `packages/worldview-map/index.html` — add `<div id="wv-health"></div>` slot in the bottom bar; add script tag
- Modify: `packages/worldview-map/src/main.js` — call `WV.HealthPanel.init()` after `WV.Controls.init()`
- Modify: `packages/worldview-map/styles/main.css` — health-panel styles
...
```bash
cd ~/projects/worldview-wt
git add packages/worldview-map/src/ui/health-panel.js \
        packages/worldview-map/index.html \
        packages/worldview-map/src/main.js \
        packages/worldview-map/styles/main.css \
...
### Task A6: Budget guard — `src/ui/budget-guard.js`

**Files:**
- Create: `packages/worldview-map/src/ui/budget-guard.js`
...
```bash
cd ~/projects/worldview-wt
git add packages/worldview-map/src/ui/budget-guard.js \
        packages/worldview-map/index.html \
        packages/worldview-map/src/main.js \
        packages/worldview-map/styles/main.css \
...
### Task A7: NWS Alerts layer (canary) — `src/layers/nws.js`

**Files:**
- Create: `packages/worldview-map/src/layers/nws.js`
...
```bash
cd ~/projects/worldview-wt
git add packages/worldview-map/src/layers/nws.js \
        packages/worldview-map/index.html \
        packages/worldview-map/src/ui/controls.js \
...
### Task B1: Plane click-track polish — `src/layers/flights.js`

Six concrete bugs.

**Files:** Modify `packages/worldview-map/src/layers/flights.js`.
...
```bash
cd ~/projects/worldview-wt
git add packages/worldview-map/src/layers/flights.js \
...
### Task B2: CCTV PiP grid — `src/ui/cctv-pip.js`

Strict DOM construction (no innerHTML).

**Files:**
- Create: `packages/worldview-map/src/ui/cctv-pip.js`
- Modify: `src/layers/cctv.js` — emit `wv-cctv-pin` on marker click
- Modify: `index.html` — `<div id="wv-pip-grid"></div>`, script tag
- Modify: `src/main.js` — init
- Modify: `styles/main.css` — tile chrome
...
```bash
cd ~/projects/worldview-wt
git add packages/worldview-map/src/ui/cctv-pip.js \
        packages/worldview-map/src/layers/cctv.js \
        packages/worldview-map/index.html \
        packages/worldview-map/src/main.js \
        packages/worldview-map/styles/main.css \
...
### Task B3: airplanes.live mil layer — `src/layers/airplanes_live.js`

Reference template for layers B4–B14.

**Files:**
- Create: `packages/worldview-map/src/layers/airplanes_live.js`
- Modify: `src/lib/sources.js` — add source
- Modify: `index.html` — script tag
- Modify: `src/ui/controls.js` — toggle
...
```bash
cd ~/projects/worldview-wt
git add packages/worldview-map/src/layers/airplanes_live.js \
        packages/worldview-map/src/lib/sources.js \
        packages/worldview-map/index.html \
        packages/worldview-map/src/ui/controls.js \
...
| **B10** Time zones | `layers/timezones.js` | static asset | local file `data/timezones.geojson` | n/a | none | UTC-offset-keyed translucent polygons |
...
unzip -p /tmp/tz.zip > ~/projects/worldview-wt/packages/worldview-map/data/timezones.geojson
ls -lh ~/projects/worldview-wt/packages/worldview-map/data/timezones.geojson
```
