/**
 * Persistent collapsible Rules panel. Ported from the card-suite v0.4.0
 * design — same layout / behavior, different localStorage namespace.
 *
 * Layout: docked to the right side of the game area on desktop (above the
 * game on narrow viewports). Default state is expanded on first mount;
 * once the user collapses it, the collapsed state is persisted in
 * localStorage keyed `werewolf.rulesCollapsed.<screen>` so they don't
 * re-dismiss it on every mount.
 */

import { RulesDoc } from '../rules-doc'

const LS_PREFIX = 'werewolf.rulesCollapsed.'
...
export function readCollapsedFlag(screen: string): boolean {
  try {
    if (typeof localStorage === 'undefined') return false
    return localStorage.getItem(LS_PREFIX + screen) === '1'
  } catch {
    return false
  }
}

export function writeCollapsedFlag(screen: string, collapsed: boolean): void {
  try {
    if (typeof localStorage === 'undefined') return
    if (collapsed) localStorage.setItem(LS_PREFIX + screen, '1')
    else localStorage.removeItem(LS_PREFIX + screen)
  } catch {
    /* ignore */
  }
}
...
export function mountRulesPanel(
  ...
  rulesPanel.dataset.screen = screen
  ...
  let collapsed = readCollapsedFlag(screen)
  ...
  toggle.addEventListener('click', () => {
    collapsed = !collapsed
    writeCollapsedFlag(screen, collapsed)
    applyCollapsed()
  })
