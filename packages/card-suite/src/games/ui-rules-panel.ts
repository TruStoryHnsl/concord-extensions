/**
 * Shared collapsible Rules panel — used by every per-game renderer.
 *
 * Layout: docked to the right side of the game area on desktop (above the
 * game on narrow viewports). Default state is expanded on first mount of a
 * game; once the user collapses it, the collapsed state is persisted in
 * `localStorage` keyed `cardSuite.rulesCollapsed.<gameId>` so they don't
 * have to re-dismiss it every time the game mounts.
 *
 * The panel is rendered into a wrapper that splits the supplied root into
 * two columns: a `gameArea` (where the game UI mounts) and the rules panel.
 */

import { RulesDoc } from './rules-doc-types'

const LS_PREFIX = 'cardSuite.rulesCollapsed.'

export interface RulesPanelHandle {
  /** The DOM element where the per-game UI should mount. */
  readonly gameArea: HTMLElement
  /** The rules panel container (right column on desktop). */
  readonly rulesPanel: HTMLElement
  /** Tear down the panel — disconnects matchMedia listener and clears DOM. */
  destroy(): void
}

/**
 * Read the collapsed flag from localStorage. Defaults to `false` (expanded)
 * if the key is missing or storage is unavailable.
 */
...
/** Write the collapsed flag. Best-effort — silently swallows storage errors. */
...
/**
 * Mount the Rules panel into `root` and return references to the game-area
 * column (where the per-game UI should render) and the rules panel itself.
 *
 * Calling code is responsible for inserting its game UI into `gameArea`,
 * NOT into `root`.
 */
export function mountRulesPanel(
  ...
  // Header row: title + toggle button. The header stays visible regardless
  // of collapsed state so the user can re-expand.
  ...
  function applyCollapsed(): void {
    if (collapsed) {
      ...
      // When collapsed, shrink the panel to a small badge.
  ...
  // Order: on desktop, rules sit to the right of the game area. On narrow
  // viewports, rules sit ABOVE the game so they're not buried below.
  if (isNarrow()) {
  ...
  // Re-layout on viewport resize (matchMedia listener). We keep the
  // listener so flipping between portrait/landscape on a tablet works.
  let mql: MediaQueryList | null = null
  const onChange = (): void => {
    // Re-order children + recompute width.
    if (isNarrow()) {
  ...
  try {
    if (typeof window !== 'undefined' && typeof window.matchMedia === 'function') {
      ...
      } else if (typeof (mql as unknown as { addListener?: (l: () => void) => void }).addListener === 'function') {
        ;(mql as unknown as { addListener: (l: () => void) => void }).addListener(onChange)
      }
    }
  } catch {
    /* matchMedia not available — fixed layout is fine */
  }
    ...
    destroy(): void {
      try {
        if (mql) {
          ...
          } else if (typeof (mql as unknown as { removeListener?: (l: () => void) => void }).removeListener === 'function') {
            ;(mql as unknown as { removeListener: (l: () => void) => void }).removeListener(onChange)
          }
        }
      } catch {
        /* ignore */
